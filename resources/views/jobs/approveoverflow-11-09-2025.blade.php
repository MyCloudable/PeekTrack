...
<x-page-template bodyClass='g-sidenav-show  bg-gray-200 dark-version'>
  ...
  <input type="button" value="Open" class="btn btn-warning" onclick="openModal('{{ $item->id }}', this)" />
    ...
    <script>
      ...
      const DRY_RUN = false; // toggle to test without server

      let _rowToRemove = null;
      ...
      const dataTableBasic = new simpleDatatables.DataTable("#datatable-basic", {
        ...
        fixedHeight: true,
        columns: [
          { select: 0, hidden: true } // hide the ID column we added
        ]
      });



      dataTableBasic.on('datatable.init', () => {
        const ds = (dataTableBasic.data && dataTableBasic.data.data) ? dataTableBasic.data.data : [];
        const rows = Array.from(dataTableBasic.body.querySelectorAll('tr'));
        console.table(rows.map(tr => {
          const di = parseInt(tr.dataset.index, 10);
          return {
            'rendered.data-index': di,
            'hiddenID(ds[di][0])': ds[di]?.[0],
            // first visible cell text (remember: hidden col 0 is NOT in DOM)
            'firstVisibleCell': tr.cells[0]?.textContent.trim()
          };
        }));
      });
    ...
    // remove via simple-datatables if possible; otherwise hide the row
    function removeCurrentRow() {
      if (!_rowToRemove) return false;

      // the tbody that simple-datatables actually renders
      const bodyRows = Array.from((dataTableBasic.body && dataTableBasic.body.rows) || []);
      const visibleIdx = bodyRows.indexOf(_rowToRemove);

      // rows API (v3 vs v4 compat)
      const rowsAPI = (typeof dataTableBasic.rows === 'function')
        ? dataTableBasic.rows()
        : dataTableBasic.rows;

      // some builds expose a mapping from visible->dataset index
      const mappedIdx = dataTableBasic.data && Array.isArray(dataTableBasic.data.activeRows)
        ? dataTableBasic.data.activeRows[visibleIdx]
        : visibleIdx;

      if (mappedIdx > -1 && rowsAPI && typeof rowsAPI.remove === 'function') {
        try {
          rowsAPI.remove(mappedIdx);           // preferred: remove via library
          if (typeof dataTableBasic.update === 'function') dataTableBasic.update();
          return true;
        } catch (_) { /* fall through */ }
      }

      // fallback: visually hide/remove the row (test mode)
      $(_rowToRemove).fadeOut(200, function () { $(this).remove(); });
      try { if (typeof dataTableBasic.update === 'function') dataTableBasic.update(); } catch (_) {}
      return true;
    }
    ...
    // Intercept modal form submit → jQuery AJAX
    $(document).on('submit', '#approvalForm', function (e) {
      e.preventDefault();

      const $form   = $(this);
      const formURL = $form.attr('action');
      const data    = $form.serialize(); // includes _token, overflow_id, decision, note
      const id      = $('#modalOverflowId').val();
      ...
      // ---- TEST PATH: don't call server, just remove row locally ----
    if (DRY_RUN) {
      console.log('[DRY_RUN] Pretending server success for id:', id);
      removeCurrentRow();
      ...
      $.ajax({
        ...
        method: 'POST',
        data: data,
        headers: { 'Accept': 'application/json' }, // triggers JSON response path
        success: function (res) {
          if (res && res.ok) {
            // Remove the row from the table (approved or rejected both disappear)
            // removeRowById(id);


            // remove the exact row the user just acted on
            if (_rowToRemove && _rowToRemove.parentNode) {

              console.log('Removing row from DOM directly:', _rowToRemove);
              console.log(_rowToRemove.parentNode);
              
              _rowToRemove.parentNode.removeChild(_rowToRemove);
              console.log('Row removed from DOM directly:', _rowToRemove);
              
              try { 
                // ask simple-datatables to redraw counts/pagination
                console.log('Updating dataTableBasic after direct DOM removal');
            ...
            // You can swap for your global toast system if available
            setTimeout(() => alert('Decision recorded.'), 0);
          } else {
        ...
        error: function (xhr) {
          ...
          const msg = xhr?.responseJSON?.message || 'Failed to submit decision.';
          alert(msg);
        },
        complete: function () {
          $submitBtn.prop('disabled', false).text('Submit');
    ...
    function openModal(overflowId, btnEl) {

      // remember the row we acted on
      _rowToRemove = btnEl ? btnEl.closest('tr') : null;
