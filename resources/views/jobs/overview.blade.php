...
<x-page-template bodyClass='g-sidenav-show  bg-gray-200 dark-version'>
            ...
            <div class="container-fluid py-4">
                <div class="row mt-4">
                    @if (auth()->user()->role_id == 10 || auth()->user()->role_id == 1)
                        <div class="col-md-12">
                            <createeditoverflow :job_id={{ $jobinfo[0]->id }} />
                        </div>
                    @endif
                    <div class="col-11">
                        <div class="card mb-4">
                                    ...
                                    <h2 class="mb-0">
                                        <table class="table table-flush" id="datatable-basic">
                                            ...
                                            <tbody>
                                                <tr>
                                                    <td>
                                                        <h4><strong>{{ $jobinfo[0]->description }}</strong></h4>
                                                    </td>
                                                    <td>
                                                        <h4><strong>{{ $jobinfo[0]->branch }}</strong></h4>
                                                    </td>
                                                    <td>
                                                        <h4><strong>{{ $jobinfo[0]->location }}</strong></h4>
                                                    </td>
                                                    <td>
                                                        <h4><strong>{{ $jobinfo[0]->contractor }}</strong></h4>
                                                    </td>
                                                </tr>
                                            </tbody>
                                        </table>
    ...
    <div class="card shadow-lg border-0">
        ...
        <div class="col-11 mt-3" style="overflow-y:scroll;max-height: 400px;">
            <div class="card mt-4">
                <div class="card-header p-3 pt-2">
                    <div
                        class="icon icon-lg icon-shape bg-gradient-warning shadow text-center border-radius-xl mt-n4 float-start">
                                ...
                                <div class="form-group form-file-upload form-file-multiple">
                                    @if (auth()->user()->role_id != 3)
                                        <button type="button" class="btn btn-warning btn-block mt-4"
                                            data-bs-toggle="modal" style="float: left;"
                                            data-bs-target="#modal-uploadfile">Upload File</button>&nbsp&nbsp&nbsp

                                        <button type="button" class="btn btn-warning btn-block mt-4"
                                            data-bs-toggle="modal" style="float: right;"
                                            data-bs-target="#modal-inactivefiles">View Archive</button>
                                    @endif
                                </div>
                            ...
                            @foreach ($files as $file)
                                @if ($file->active == '0' && $file->type != 'Work Order')
                                    <tr>
                                        ...
                                        <td class="font-weight-bolder opacity-9">
                                            <h5>{{ $file->description }}</h5>
                                        </td>
                                        <td class="td-name">
                                            <h5>{{ $file->created_at }}</h5>
                                        </td>
                                        <td><input type="button" value="View" class="btn btn-warning"
                                                onclick="window.open('/uploads/{{ $file->name }}');"></td>
                                        <td>
                                            @if (auth()->user()->role_id != 3)
                                                <a href="/delete-file/{{ $file->id }}" class="btn btn-danger"
                                                    onclick="return confirm('Are you sure you want to archive this job document?');">ARCHIVE</a>
                                            @endif
                                        </td>
                                    </tr>
                                @endif
                                ...
                                @if ($file->active == '0' && $file->type == 'Work Order')
                                    <tr>
                                        ...
                                        <td>
                                            @if (auth()->user()->role_id != 3)
                                                <a href="/delete-file/{{ $file->id }}" class="btn btn-danger"
                                                    onclick="return confirm('Are you sure you want to archive this work order?');">ARCHIVE</a>
                                            @endif
                                        </td>
                                    </tr>
                                @endif
        ...
        <br>
        <button type="button" class="btn btn-round bg-gradient-warning mb-1" style="margin-left: 73%;"
            data-bs-toggle="modal" data-bs-target="#modal-notification">New Job Card Entry</button>
                                    ...
                                    <tr>
                                        <td>
                                            @if ($jobentry->submitted == 1)
                                                <input type="button" class="btn btn-round bg-gradient-warning mb-3"
                                                    value="View"
                                                    onclick="location.href='/jobs/{{ $jobentry->link }}/view/';">
                                            @else
                                                <input type="button" class="btn btn-round bg-gradient-warning mb-3"
                                                    value="Open"
                                                    onclick="location.href='/jobs/{{ $jobentry->link }}/jobcard/';">
                                            @endif
                                        </td>
                                        <td>
                                            <h4>{{ $jobentry->submitted == 1 ? 'Yes' : 'No' }}</h4>
                                        </td>
                                        <td>
                                            <h4>
...
<div class="modal fade" id="modal-notification" tabindex="-1" role="dialog" aria-labelledby="modal-notification" aria-hidden="true">
      <div class="modal-dialog modal-dialog-centered" role="document">
        ...
        <div class="modal fade" id="modal-inactivefiles" tabindex="-1" role="dialog"
        ...
        <div class="modal fade" id="modal-uploadfile" tabindex="-1" role="dialog"
            aria-labelledby="modal-notification" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        ...
                        <div class="py-3 text-center">
                            ...
                            <form action="{{ route('fileUpload') }}" method="post" enctype="multipart/form-data">
                                ...
                                <input class="form-control" type="text"
                                    style="color: black;border-spacing: 2px;display: table-cell; vertical-align: inherit;background-color: white;width: 100%;margin-bottom: 1rem;vertical-align: top;border-color: #f0f2f5;"
                                    name="docdesc" placeholder="Description" required>
        ...
        <script>
            $(document).ready(function() {
                $('#datatable-jobentry').DataTable({
                    order: [
                        [3, 'desc']
                    ], // Default sort by Work Date (4th column, index 3)
                    language: {
