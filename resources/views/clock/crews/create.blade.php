                ...
                <div class="card-body pt-0">
                    <form method="POST" action="{{ route('crews.store') }}">
                        ...
                        <div class="input-group input-group-outline">
                            ...
                            <input type="text" name='crew_name' class="form-control" value="{{old('crew_name')}}">
                        </div>
                        @error('crew_name')
                        ...
                        <div class="input-group input-group-outline mt-4">
                            <select name="superintendentId" class="form-control" id="exampleFormControlSelect1">
                                ...
                                <option value="{{$user->id}}" {{ $user->id === old('superintendentId') ? 'selected' : '' }}>{{$user->email}}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="input-group input-group-outline mt-4 position-relative">
                            <select class="selectpicker multiselect" multiple data-live-search="true" name="crew_members[]">
                                @foreach ($users as $user)
                                <option value="{{$user->id}}" {{ $user->id === old('crew_members') ? 'selected' : '' }}>{{$user->email}}</option>
                                @endforeach
                            </select>
                        </div>
                        @error('crew_members')
                        ...
                        <button class="btn bg-gradient-dark btn-sm mt-6 mb-0">Submit</button>
                    </form>
                </div>
            </div>
         </div>
