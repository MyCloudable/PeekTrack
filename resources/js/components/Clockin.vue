        ...
        <div class="modal-dialog modal-fullscreen" role="document">
            <div class="modal-content clk-shell"
                ...
                <!-- ══════════════════════════════════════════════════════
                     ...
                     ══════════════════════════════════════════════════════ -->
                <div class="clk-rotate" v-if="needsRotation" role="alertdialog"
                    aria-labelledby="clk-rotate-title" aria-describedby="clk-rotate-desc">
                        ...
                        <div class="clk-rotate__icon" aria-hidden="true">
                            <svg viewBox="0 0 120 120" width="96" height="96">
                                ...
                                <g class="clk-rotate__phone">
                                    <rect x="44" y="18" width="32" height="60" rx="5" ry="5"
                                ...
                                <path d="M 20 95 Q 60 110 100 95"
                                    fill="none" stroke="currentColor" stroke-width="2.5"
                                    stroke-linecap="round" stroke-dasharray="4 4" />
                                <polyline points="95,88 100,95 93,100"
                                    fill="none" stroke="currentColor" stroke-width="2.5"
                    ...
                    <div class="clk-body clk-body--phone">
                        ...
                        <div class="clk-phone-left">
                            ...
                            <div class="clk-phone-field" v-if="canSwitchTypes">
                                ...
                                <button class="clk-btn clk-btn--outline mt-2" @click="switchTimeType" :disabled="isBusy">
                                    Apply
                                </button>
                            </div>
                        ...
                        <div class="clk-phone-crew-section">
                            <div class="clk-phone-crew-head">
                                ...
                                <div v-if="isAlreadyClockedin" class="clk-phone-crew-head-right">
                                    <half-full-per-diem :timesheetId="allPerDiemTimesheetIds" :perDiem="allPerDiemStatus"
                ...
                <footer class="clk-actionbar">
                    ...
                    <button v-if="!isAlreadyVerified" class="clk-btn clk-btn--primary clk-btn--hero"
                        @click="verifyTeam" :disabled="isBusy">
...
.clk-shell {
    ...
    overflow: hidden;
    position: relative; /* anchor for the rotation overlay */
}
...
.clk-body {
    flex: 1;
    min-height: 0; /* allow the flex child to shrink so overflow-y actually kicks in */
    overflow-y: auto;
...
.clk-card--compact { padding: 0; overflow: hidden; }
.clk-card--flush   { padding: 14px 0; }
...
.clk-collapse-toggle:hover { background: rgba(255, 255, 255, 0.02); }
.clk-collapse-toggle.is-open { background: rgba(59, 130, 246, 0.08); }
.clk-collapse-toggle.is-disabled { opacity: 0.5; pointer-events: none; }
...
.clk-collapse-toggle__left i { color: var(--clk-primary); }
...
.clk-select:disabled { opacity: 0.5; cursor: not-allowed; }
...
.clk-inline-pair .clk-select { flex: 1; }

/* Datepicker */
.clk-datepicker { width: 100%; }
...
:global(.dp--menu-wrapper) { z-index: 1080 !important; }
:global(.dp--clear-btn) { display: none !important; }
...
.clk-btn:active:not(:disabled) { transform: scale(0.97); }
.clk-btn:disabled { opacity: 0.55; cursor: not-allowed; }
.clk-btn:focus-visible { outline: none; box-shadow: 0 0 0 3px rgba(59, 130, 246, 0.35); }

.clk-btn--sm { min-height: 44px; padding: 8px 14px; font-size: 0.85rem; }
...
.clk-btn--primary { background: var(--clk-primary); color: #fff; }
.clk-btn--primary:hover:not(:disabled) { background: #2563eb; }

.clk-btn--success { background: var(--clk-success); color: #fff; }
.clk-btn--success:hover:not(:disabled) { background: #16a34a; }

.clk-btn--danger { background: var(--clk-danger); color: #fff; }
.clk-btn--danger:hover:not(:disabled) { background: #dc2626; }
...
.clk-btn--ghost:hover:not(:disabled) { background: var(--clk-border); }
...
.clk-add-crew :deep(.select2-container) { width: 100% !important; }
...
.clk-add-crew :deep(.select2-selection__arrow) { height: 46px !important; }
...
.clk-empty p { margin: 0; font-size: 0.9rem; }
...
.clk-crew-row:first-child { border-top: none; }
.clk-crew-row.is-checked { background: rgba(59, 130, 246, 0.06); }
.clk-crew-row.is-out     { opacity: 0.7; }

.clk-crew-row__check { padding-top: 2px; margin: 0; }
...
.clk-crew-row__times { margin-top: 4px; }
...
.clk-icon-btn:disabled { opacity: 0.4; cursor: not-allowed; }
...
.clk-chip--muted { color: var(--clk-muted); }
.clk-chip--in    { background: rgba(34, 197, 94, 0.15); border-color: rgba(34, 197, 94, 0.35); color: #86efac; }
.clk-chip--out   { background: rgba(239, 68, 68, 0.12); border-color: rgba(239, 68, 68, 0.3);  color: #fca5a5; }
...
@media (min-width: 768px) {
    .clk-body:not(.clk-body--phone) { padding: 20px; }
    .clk-add-crew { grid-template-columns: 1.5fr 1.3fr auto; align-items: center; }
    .clk-time-edit { grid-template-columns: 1fr 1fr; }
}
...
.clk-shell--phone .clk-topbar__title i { font-size: 1.05rem; }
...
.clk-phone-late-entry i:first-child { color: var(--clk-primary); }
.clk-phone-late-entry.is-open { background: rgba(59, 130, 246, 0.12); border-color: var(--clk-primary); }
.clk-phone-late-entry.is-disabled { opacity: 0.5; pointer-events: none; }
...
.clk-phone-addcrew-body :deep(.select2-container) { width: 100% !important; }
...
.clk-phone-crew-section {
    ...
    border-radius: var(--clk-radius);
    overflow: hidden;
    min-height: 120px; /* always reserve space for at least one crew card + head */
    flex-shrink: 0;    /* never collapse below min-content */
}
...
.clk-phone-crew-card:last-child { border-bottom: none; }
.clk-phone-crew-card.is-checked { background: rgba(59, 130, 246, 0.08); }
.clk-phone-crew-card.is-out     { opacity: 0.75; }
...
.clk-phone-crew-card__check { margin: 0; flex-shrink: 0; }
...
.clk-phone-crew-card__meta {
    ...
    flex-wrap: wrap;
    gap: 4px;
    padding-left: 32px; /* align with name, past checkbox */
}
...
@media (max-width: 360px) {
    .clk-shell--phone .clk-topbar__title { font-size: 0.85rem; }
    .clk-shell--phone .clk-status__pill { font-size: 0.85rem; padding: 7px 12px; }
    .clk-shell--phone .clk-btn--hero { font-size: 0.9rem; min-height: 52px; }
    .clk-shell--phone .clk-phone-crew-card__name { font-size: 0.9rem; }
    .clk-shell--phone .clk-phone-crew-card__edit { grid-template-columns: 1fr; }
    .clk-shell--phone .clk-body--phone { padding: 10px 10px; gap: 8px; }
}
...
@media (orientation: landscape) and (max-height: 500px) {
    ...
    .clk-shell--phone .clk-topbar__title { font-size: 0.8rem; }
    .clk-shell--phone .clk-topbar__title i { font-size: 0.95rem; }
    .clk-shell--phone .clk-topbar__close { width: 30px; height: 30px; font-size: 0.85rem; }
    ...
    .clk-shell--phone .clk-body--phone > .clk-body__spacer { display: none; }
    ...
    .clk-shell--phone .clk-phone-crew-title { font-size: 0.9rem; }
    .clk-shell--phone .clk-phone-crew-card {
        padding: 7px 10px;
        gap: 3px;
    }
    .clk-shell--phone .clk-phone-crew-card__name { font-size: 0.88rem; }
    .clk-shell--phone .clk-phone-crew-card__meta { padding-left: 0; }
...
@media (max-height: 600px) {
    .clk-rotate__icon { width: 88px; height: 88px; }
    .clk-rotate__icon svg { width: 72px; height: 72px; }
    .clk-rotate__title { font-size: 1.15rem; }
    .clk-rotate__desc  { font-size: 0.85rem; }
    .clk-rotate__inner { gap: 14px; }
}
</style>
...
#clockin .modal-dialog {
    ...
    height: 100dvh !important; /* modern mobile-safe viewport unit */
    margin: 0 !important;
